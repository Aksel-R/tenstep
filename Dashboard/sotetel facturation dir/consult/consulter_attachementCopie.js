...
<div class="mb-1 text-muted"><center><strong style="font-size: large;">Responsable sotetel</strong></center></div>
      ...
      <center>
             ...
             <button class="btn btn-primary hidden-print" id="CSVWindows" onclick="return false;"><span class="glyphicon glyphicon-file" aria-hidden="true"></span> Enregistrer CSV</button>
...
/*$("#glyphiconConsult").click(function () {
  ...
  $(document).ready(function() {
    $("#CSVWindows").on("click", function() {
      exportAllTablesToCSV();
    });

    function exportAllTablesToCSV() {
      let csvContent = "";
      const tables = $("#PrintFacture").find("table");

      // Loop through each table and extract data from cells
      tables.each(function(tableIndex, table) {
        const rows = $(table).find("tr");

        // Loop through each row and get the data from cells
        ...
        // Add an empty line between tables to separate them in the CSV
        csvContent += "\n";
      });
      ...
      downloadLink.download = "tables_data.csv";
