...
function gotProperty() {                
           ...
           var urlTaskName = _spPageContextInfo.webServerRelativeUrl + "/_api/ProjectData/[en-US]/Projects(guid'1191c36b-f784-ec11-889c-00155dde6963')/Tasks?$select=TaskName,Zone,TaskId&$filter=Zone eq 'Zone 4'";
...
// });
// }
// _spBodyOnLoadFunctionNames.push("getZone");

var responseAss = responseAss ||[];
var urlAss = _spPageContextInfo.siteAbsoluteUrl + "/_api/ProjectData/[en-US]/Projects(guid'1191c36b-f784-ec11-889c-00155dde6963')/Assignments?$select=ResourceName,TaskName,ResourceId,TaskId&$filter=ResourceName ne 'Ressource non affectée' and ResourceName ne 'Ressource locale générique'";
function filterAss(){
          $.ajax({url:  urlAss,          
                   method: "GET",
                   dataType: "json",
...
// function ressourceProj(resIdArr,resourceNom){
//   var idRes;
// var resNom;
// var tasknm ;
// var zoneTb;
...
//      UrlArticle = _spPageContextInfo.siteAbsoluteUrl + "/_api/ProjectData/[en-US]/Resources?$select=ResourceId,ResourceName,RessourceType,CodeArticle,Unité,Quantite,Prix_unit_sud&$filter=ResourceId%20eq%20guid%27"+idRes+"%27 and ResourceIsGeneric eq false and CodeArticle ne null and ResourceName%20ne%20%27Ressource%20non%20affectée%27%20and%20ResourceName%20ne%20%27Ressource%20locale%20générique%27 and RessourceType eq 'Article'";
...
function mainTab(nomresId,residArticle) {
  UrlArticle = _spPageContextInfo.siteAbsoluteUrl + "/_api/ProjectData/[en-US]/Resources?$select=ResourceId,ResourceName,RessourceType,CodeArticle,Unité,Quantite,Prix_unit_nord&$filter=ResourceId%20eq%20guid%27"+residArticle+"%27 and ResourceIsGeneric eq false and CodeArticle ne null and ResourceName%20ne%20%27Ressource%20non%20affectée%27%20and%20ResourceName%20ne%20%27Ressource%20locale%20générique%27 and RessourceType eq 'Article'";
...
function loadTab(ARRtypeRes,ARRcodeRes,ARRnomRes,ARRunite,ARRquantpr,ARRprix_unites,ARRnomprojet,ARRres1,ARRFTTC) {
  for (var i = 0; i < ARRcodeRes.length; i++) {
      ...
      trSuiv.append("<td class='puc' style='display:none;'><input id='pu' type='text'  style='width: 80px; height: 30px border:none; text-align:center;' readonly class='refresh_prices row_quantity2'/></td>");
      trSuiv.append("<td class='pus' style='display:none;'><input id='pu'value="+ parseFloat(ARRprix_unites[i]).toFixed(3) +" type='text'  style='width: 80px; height: 30px border:none; text-align:center;' readonly class='refresh_prices row_quantity3'/></td>");
      ...
      trSuiv.append("<td class='zoneArticle' style='display:none;'>Zone 4</td>");
