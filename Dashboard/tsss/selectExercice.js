function loadListsByExercice(){
  ...
  var exerciseSel=document.getElementById("mylist4").value;
  var exerciseCh="";
  	if (exerciseSel!="") {
  		var nextYear=moment(exerciseSel).add(1, 'years').format('YYYY-MM-DD');
  		exerciseCh+=" and ProjectStartDate%20ge%20datetime%27"+exerciseSel+"T00:00:00.000%27 and ProjectStartDate%20lt%20datetime%27"+nextYear+"T00:00:00.000%27"
	var responsePortfolioListFilter=responsePortfolioListFilter||[];
	var urlPortfolioListFilter=_spPageContextInfo.siteAbsoluteUrl + "/_api/ProjectData/[en-US]/Projects?$select=EnterpriseProjectTypeName&$filter=ProjectName%20ne%20%27Opérations%20d’administration%20de%20feuilles%20de%20temps%27"+exerciseCh;

	function LoadPortfolioListFilter() {
            var nbre = 0;
            $.ajax({
                url: urlPortfolioListFilter,
                method: "GET",
                dataType: "json",
                headers: { Accept: "application/json;odata=verbose" },
                success: function (data) {
                    responsePortfolioListFilter = responsePortfolioListFilter.concat(data.d.results);
                    if (data.d.__next) {
                        urlPortfolioListFilter = data.d.__next;
                        LoadPortfolioListFilter();
                    }
                    if (!data.d.__next){
                    	
                    	
	                    $.each(responsePortfolioListFilter, function (key, value) {
	...
	var urlProgListFilter=_spPageContextInfo.siteAbsoluteUrl + "/_api/ProjectData/[en-US]/Projects?$select=Programme,ProjectId,ProjectName&$filter=ProjectName%20ne%20%27Opérations%20d’administration%20de%20feuilles%20de%20temps%27"+exerciseCh;
	...
	var urlProjFilterSel1=_spPageContextInfo.siteAbsoluteUrl + "/_api/ProjectData/[en-US]/Projects?$select=Programme,ProjectId,ProjectName&$filter=ProjectName%20ne%20%27Opérations%20d’administration%20de%20feuilles%20de%20temps%27"+exerciseCh;
