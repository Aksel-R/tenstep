...
{var args=RGraph.getArgs(arguments,'object,text,x,y,index,event');if(RGraph.SHOW_TOOLTIP_TIMER){clearTimeout(RGraph.SHOW_TOOLTIP_TIMER);}
...
+' '+value+'</td></tr>';}
str=str.join('');text=text.replace('%{key}','<table style="color: inherit" id="rgraph_tooltip_key" class="rgraph_tooltip_key RGraph_tooltip_key">'+str+'</table>');})();text=text.replace(/%{index}/g,specific.index);text=text.replace(/%{dataset2}/g,specific.dataset2);text=text.replace(/%{dataset}/g,specific.dataset);text=text.replace(/%{group2}/g,specific.dataset2);text=text.replace(/%{group}/g,specific.dataset);text=text.replace(/%{sequential_index}/g,specific.sequentialIndex);text=text.replace(/%{seq}/g,specific.sequentialIndex);if(text.indexOf('%{list}')!==-1){(function()
...
tooltipObj.__original_text__=args.text;args.text=substitute(args.text);if(args.object.properties.tooltipsPointer){args.text+='<div id="RGraph_tooltipsPointer" style="background-color:black; color: transparent;position:absolute;bottom:-5px;left:50%;transform:translateX(-50%) rotate(45deg);width:10px;height:10px"></div>';if(!RGraph.isNull(args.object.properties.tooltipsPointerCss)){setTimeout(function()
...
if(RGraph.isFixed(args.object.canvas)){var scrollTop=window.scrollY||document.documentElement.scrollTop;tooltipObj.style.position='fixed';tooltipObj.style.top=args.event.pageY-scrollTop-height-10+args.object.properties.tooltipsOffsety+'px';}
