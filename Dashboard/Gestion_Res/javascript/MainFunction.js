function MainFunction()
{
	//Les variables qu'on va utiliser
		...
		var numproj=0;
		var urlGetProjects =  _spPageContextInfo.siteAbsoluteUrl + "/_api/ProjectData/[en-US]/Projects?$filter=ProjectName%20ne%20%27Opérations%20d’administration%20de%20feuilles%20de%20temps%27";
...
function DoughnutLineChart()
  {
  	 var date = new Date();
            var yyyy = date.getFullYear();
      $.ajax({
    url: _spPageContextInfo.webAbsoluteUrl + "/_api/ProjectData/[en-US]/ResourceTimephasedDataSet?$filter=year(TimeByDay)%20eq%20"+yyyy+"%20and%20ResourceName%20eq%20%27"+nomress+"%27",
    ...
    success: function (data) {
        ...
        if (data.value.length > 0) {
           ...
           var reducedObject = {};
            ...
            var ctx = document.getElementById("Singlebar");

			if (window.myPieChartLine != undefined)
			{
			    window.myPieChartLine.destroy();
			}

            window.myPieChartLine = new Chart(ctx, {
  ...
  labels: {
	...
	{
			$.ajax({
    url: _spPageContextInfo.webAbsoluteUrl + "/_api/ProjectData/[en-US]/Projects?$filter=ProjectOwnerName%20eq%20%27"+nomress+"%27",
...
function BarsLineChart()
	...
	//**************************** Bar ************************************			
		   ...
		   var reducedObject2 = {};
            ...
            for (var i = 0; i < data.value.length; i++) {
                    if (!reducedObject1[data.value[i].Category]) {
					
                        reducedObject1[data.value[i].Category] = 0;
                }
				if (!reducedObject2[data.value[i].Category]) {
					
                        reducedObject2[data.value[i].Category] = 0;
                }
                reducedObject1[data.value[i].Category] += parseInt(data.value[i].Cost);
