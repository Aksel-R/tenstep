...
<h4 style="text-align:left;font-weight: bolder;margin: 0px">Ressource Participante à la feuille de temps</h4>
<select id="resourcePart" style="width:100%;" multiple="multiple" onchange="mainFilter1();"></select>
    ...
    <script src="/DCSIPWA/_catalogs/masterpage/Dashboard/Detail_timesheet/javascript/timesheeteFilter.js"></script>
    <script src="/DCSIPWA/_catalogs/masterpage/Dashboard/Detail_timesheet/javascript/MainFunction.js"></script>
...
//Load Timesheet Line
var urlLoadTimeSheetLine = _spPageContextInfo.siteAbsoluteUrl + "/_api/ProjectData/[en-US]/TimesheetLines?$select=TimesheetClassName";
var responseTimeSheetLine = responseTimeSheetLine ||[]; 

        function TimesheetLine()
        {
            $.ajax({
                url: urlLoadTimeSheetLine,
                method: "GET",
                dataType: "json",
                headers: { Accept: "application/json;odata=verbose" },
                ...
                /*success: function (data) {
                    ...
                    success: function(data) {     
         responseTimeSheetLine = responseTimeSheetLine.concat(data.d.results);
           if (data.d.__next) {
             urlLoadTimeSheetLine = data.d.__next;
             TimesheetLine();
          } 

                    $.each(responseTimeSheetLine, function (key, value) {
                        var category = value.TimesheetClassName;
                        if (category) {
                            $('#categorie').append($('<option >',
                                {
...
var urlLoadTimeSheetLineTypeOP = _spPageContextInfo.siteAbsoluteUrl + "/_api/ProjectData/[en-US]/TimesheetLines?$select=ProjectName";
var responseTimeSheetLineTypeOP = responseTimeSheetLineTypeOP ||[]; 


        function TimesheetLineTypeOperation()
        {

                    $.ajax({
                url: urlLoadTimeSheetLineTypeOP,
                method: "GET",
                dataType: "json",
                headers: { Accept: "application/json;odata=verbose" },
                success: function (data) {
                    responseTimeSheetLineTypeOP = responseTimeSheetLineTypeOP.concat(data.d.results);
           if (data.d.__next) {
             urlLoadTimeSheetLineTypeOP = data.d.__next;
             TimesheetLineTypeOperation();
          } 
                 
                    $.each(responseTimeSheetLineTypeOP, function (key, value) {
