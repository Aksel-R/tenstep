...
alert(error);
	...
	function LoadMesAgences() {
                ...
                success: function (data) {
                    ...
                    if (!data.d.__next){
	                   ...
	                   ////console.log("arrMesAgences= "+arrMesAgences);
	                    ...
	                    removeFromSelect(mySelect, arrMesAgences,arrId);
      ...
      function removeFromSelect(yourSelect, arrOptionsVal,arrId)
		{
		    // $(yourSelect).find('option').filter(function(){
		    //      return arrOptionsVal.indexOf(this.text) == -1;
		    // }).remove();
		    $(yourSelect).empty();
        	for (var i = 0; i < arrOptionsVal.length; i++) {
        		
        		$(yourSelect).append('<option value="'+arrId[i]+'" selected>'+arrOptionsVal[i]+'</option>');
        	}
